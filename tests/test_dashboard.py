...
@pytest.fixture
def client():
    """Create a TestClient with mocked ppmlx modules for dashboard tests."""
    _mock_engine = MagicMock()
    _mock_engine.generate.return_value = ("Hello!", None, 10, 5)
    _mock_engine.stream_generate.return_value = iter(["Hello", " ", "world"])
    _mock_engine.list_loaded.return_value = ["mlx-community/Llama-3-8B"]
    _mock_tokenizer = MagicMock()
    _mock_tokenizer.has_tool_calling = False
    _mock_engine.get_tokenizer.return_value = _mock_tokenizer

    _mock_embed_engine = MagicMock()
    _mock_embed_engine.encode.return_value = [[0.1, 0.2, 0.3]]

    _mock_db = MagicMock()
    _mock_db.get_stats.return_value = {
        "total_requests": 42,
    ...
    _mock_db.query_requests.return_value = [
        {
            "id": 1,
    ...
    _mock_db._path = "/tmp/test_ppmlx.db"
    _mock_db.query_time_series.return_value = []

    _mock_config = MagicMock()
    _mock_config.logging = SimpleNamespace(snapshot_interval_seconds=60)
    _mock_config.tool_awareness = SimpleNamespace(mode="no_tools_only")

    sys.modules["ppmlx.engine"].get_engine = MagicMock(return_value=_mock_engine)
    sys.modules["ppmlx.engine_embed"].get_embed_engine = MagicMock(return_value=_mock_embed_engine)
    sys.modules["ppmlx.db"].get_db = MagicMock(return_value=_mock_db)
    sys.modules["ppmlx.memory"].get_system_ram_gb = MagicMock(return_value=36.0)
    sys.modules["ppmlx.config"].load_config = MagicMock(return_value=_mock_config)
